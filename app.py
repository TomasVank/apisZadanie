...
def UpdateOrder(id):
  ...
  myDb = MYSQL.connect(host="147.232.40.14", user="tv635vg", passwd="Airi8Eiw", database="tv635vg")
  cursor = myDb.cursor()
  cursor.execute(sql.format(order_dict["product_ID"],order_dict["customer_ID"],order_dict["orders_quantity"],order_dict["orders_ID"]))
  myDb.commit()
  cursor.close()
  myDb.close()
...
def UpdateCustomer(id):
  ...
  myDb = MYSQL.connect(host="147.232.40.14", user="tv635vg", passwd="Airi8Eiw", database="tv635vg")
  cursor = myDb.cursor()
  cursor.execute(sql.format(customer_dict["customer_firstName"],customer_dict["customer_lastName"],customer_dict["customer_phone"],customer_dict["customer_email"],customer_dict["customer_address"],customer_dict["customer_ID"]))
  myDb.commit()
  cursor.close()
  myDb.close()
  return jsonify("Update"),200
...
def UpdateProduct(id):
  ...
  myDb = MYSQL.connect(host="147.232.40.14", user="tv635vg", passwd="Airi8Eiw", database="tv635vg")
  cursor = myDb.cursor()
  cursor.execute(sql.format(product_dict["product_name"],product_dict["product_price"],product_dict["product_pieces_WH"],product_dict["product_ID"]))
  myDb.commit()
  cursor.close()
  myDb.close()
  return jsonify("Update"),200
